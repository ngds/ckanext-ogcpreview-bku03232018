...
//***********************************************************************************************
//@NGDS Added a View for Histograms

(function($, my) {

	// ## Histogram Graph view for a Dataset using Flot graphing library.
	//
	// Initialization arguments (in a hash in first parameter):
	//
	// * model: recline.Model.Dataset
...
//	}
	...
	// NB: should *not* provide an el argument to the view but must let the view
	// generate the element itself (you can then append view.el to the DOM.
	my.FlotHisto = Backbone.View.extend({
		template: ' \
			<div class="recline-flot"> \
			<div class="panel graph" style="display: block;"> \
			<div class="js-temp-notice alert alert-block"> \
			<h3 class="alert-heading">Hey there!</h3> \
			<p>There\'s no Flot histogram graph here yet because we don\'t know what fields you\'d like to see plotted.</p> \
			<p>Please tell us by <strong>using the menu on the right</strong> and a graph will automatically appear.</p> \
			</div> \
			</div> \
			</div> \
			...
			initialize: function(options) {
				...
				this.state = new recline.Model.ObjectState(stateData);
				this.previousTooltipPoint = {x: null, y: null};
				this.editor = new my.FlotHistoControls({
					model: this.model,
					state: this.state.toJSON()
				});
				this.editor.state.bind('change', function() {
			...
			redraw: function() {
				...
				if (this.state.get('group') && this.state.get('series')) {
					...
					console.log(options);
					/* */
					this.plot = $.plot(this.$graph, [ series[0].bins ], {
						series: {
							bars: {
								show: true,
								barWidth: 0.6,
			...
			//
			// param: series	- The data series for which to create the histogram (can be more than one)
			//
	...
	my.FlotHistoControls = Backbone.View.extend({
		className: "editor",
		template: ' \
			<div class="editor"> \
			<form class="form-stacked"> \
			...
			<div class="editor-buttons"> \
			<button class="btn editor-add"  comment="hidden temporarily" style="display: none;">Add Series</button> \
			</div> \
			<div class="editor-buttons editor-submit" comment="hidden temporarily" style="display: none;"> \
			<button class="editor-save">Save</button> \
			<input type="hidden" class="editor-id" value="chart-1" /> \
			...
			templateSeriesEditor: ' \
				...
				<input id="i_precision"    type="number" maxlength="2" size="2" value="3"/>\
				<label>Number of Bins:</label>\
				<input id="i_numBins" type="number" maxlength="2" size="2" value="4"/>\
				<div id="bin_slider"></div>\
				</div> \
				</div> \
			',
				events: {
					'change form select': 'onEditorSubmit',
					'change input': 'onEditorInputSubmit',
					'click .editor-add': '_onAddSeries',
					'click .action-remove-series': 'removeSeries'
				},
				...
				onEditorSubmit: function(e) {
					...
					var updatedState = {
							...
							group: this.el.find('.editor-group select').val(),
							graphType: this.el.find('.editor-type select').val(),
							numBins: $('#i_numBins').val(),
							precision: $('#i_precision').val()
					};
					this.state.set(updatedState);
				},
...
this.recline.View.GraphHisto = this.recline.View.FlotHisto;

this.recline = this.recline || {};
this.recline.View = this.recline.View || {};
//this.recline.View.Graph = this.recline.View.FlotHisto;
//this.recline.View.GraphControls = this.recline.View.FlotHistoControls;
